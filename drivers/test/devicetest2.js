  ...
  onInit() {
    ...
    socket.on('connect', () => {
      ...
      this.pollingInterval = setInterval(() => {
          ...
          client.readHoldingRegisters(40242, 1), //4 Scale factor totaal
          ...
          Promise.all([
            ...
            client.readHoldingRegisters(40210, 1), //4 powerscale importexport meter
            client.readHoldingRegisters(40226, 2), //6 total exported power
            client.readHoldingRegisters(40234, 2), //7 total imported power
            client.readHoldingRegisters(40084, 1) //8 Scale factor importexport

          ]).then((results) => {
          var powergrid = results[6].response._body._valuesAsArray[0];
          var meterscale = results[7].response._body._valuesAsBuffer;
          var totalexport = results[8].response._body._valuesAsBuffer;
          var totalimport = results[8].response._body._valuesAsBuffer;
          var impexpscale = results[10].response._body._valuesAsBuffer;
          this.log('Powergrid', powergrid);
          //11 meter id 1
          //logs
